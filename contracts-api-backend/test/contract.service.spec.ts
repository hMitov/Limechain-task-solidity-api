  ...
  ethers: {
    ...
    Contract: jest.fn().mockImplementation(() => ({
      ...
      isWhitelisted: jest.fn().mockImplementation((address) => {
        return Promise.resolve(address === '0x1111111111111111111111111111111111111111');
      }),
      addToWhitelist: jest.fn().mockResolvedValue({ wait: jest.fn().mockResolvedValue({}) }),
      removeFromWhitelist: jest.fn().mockResolvedValue({ wait: jest.fn().mockResolvedValue({}) }),
      updatePrice: jest.fn().mockResolvedValue({ wait: jest.fn().mockResolvedValue({}) }),
      updatePublicPrice: jest.fn().mockResolvedValue({ wait: jest.fn().mockResolvedValue({}) }),
      updatePrivatePrice: jest.fn().mockResolvedValue({ wait: jest.fn().mockResolvedValue({}) }),
      setPrices: jest.fn().mockResolvedValue({ wait: jest.fn().mockResolvedValue({}) }),
    })),
...
describe('ContractService', () => {
  ...
  describe('fetchAvailableNFTs', () => {
    ...
    it('should handle database errors', async () => {
      mockNftRepo.find.mockRejectedValue(new Error('Database connection failed'));

      await expect(service.fetchAvailableNFTs()).rejects.toThrow('Database connection failed');
    });
  ...
  describe('fetchOngoingAuctions', () => {
    ...
    it('should handle database errors', async () => {
      mockAuctionRepo.find.mockRejectedValue(new Error('Database connection failed'));

      await expect(service.fetchOngoingAuctions()).rejects.toThrow('Database connection failed');
    });
  ...
  describe('Whitelist Management', () => {
    ...
    it('should validate whitelist addresses format', async () => {
      ...
      await expect(
        service.addToWhitelist(mockOwner, invalidAddresses)
      ).rejects.toThrow(BadRequestException);
    });

    it('should handle empty whitelist addresses array', async () => {
      const result = await service.removeFromWhitelist(mockOwner, []);
      expect(result).toEqual({
        success: true,
  ...
  describe('Network Connection', () => {
    it('should validate network connection before operations', async () => {
      mockContract.getNetwork = jest.fn().mockRejectedValue(new Error('Network error'));
      ...
      await expect(
        service.updateNftPrices(mockOwner, '0.1', '0.2')
      ).rejects.toThrow('Update NFT prices failed after 3 attempts: Cannot read properties of undefined (reading \'wait\')');
    });
  });
});